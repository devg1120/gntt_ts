 ...
 */
export function createTrackedMemo(fn: any, name: any, options: any) {
 ...
 */
export function getMemoStat(name: any) {
    const stats = memoRegistry.get(name);
...
export function detectCascades() {
    ...
    for (const [frameId, invocations] of byFrame) {
        if (invocations.length > 1) {
            const uniqueMemos = new Set(invocations.map((i: any) => i.name));
            const totalTime = invocations.reduce((sum: any, i: any) => sum + i.duration, 0);
            ...
            cascades.push({
                ...
                invocations: invocations.sort((a: any, b: any) => a.timestamp - b.timestamp),
            });
        }
    }
 ...
 */
export function trackFunction(fn: any, name: any) {
    if (!memoRegistry.has(name)) {
        memoRegistry.set(name, {
            name,
            callCount: 0,
    ...
    return function tracked(this: any, ...args: any[]) {
        const start = performance.now();
