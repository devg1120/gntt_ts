 ...
 */
export function createBenchmarkResult(options: any) {
    const {
        frameAnalysis = {},
        memoAnalysis = {},
        config = {},
 ...
 * @returns {string}
 */
export function exportJSON(result: any, options = {}) {
    // @ts-expect-error TS(2339): Property 'pretty' does not exist on type '{}'.
 ...
 */
export function downloadJSON(result: any, filename: any) {
 ...
 */
export function storeBenchmark(result: any, key = 'gantt-benchmark-latest') {
    try {
 ...
 */
export function compareBenchmarks(baseline: any, current: any) {
    const comparison = {
        metadata: {
...
function round(num: any, decimals = 2) {
