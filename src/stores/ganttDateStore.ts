...
export function createGanttDateStore(options = {}) {
    ...
    // @ts-expect-error TS(2339): Property 'viewMode' does not exist on type '{}'.
    ...
    // @ts-expect-error TS(2339): Property 'viewModes' does not exist on type '{}'.
     ...
     */
    const setupDates = (tasks: any, infinitePadding = false) => {
        if (!tasks || tasks.length === 0) {
            // Default to today +/- padding
            const today = date_utils.today();
            const mode = viewMode();
            // @ts-expect-error TS(2339): Property 'duration' does not exist on type '{ dura... Remove this comment to see the full error message
     ...
     */
    const extendTimeline = (direction: any, units = 10) => {
        const stepVal = step();
        const unitVal = unit();
    ...
    const changeViewMode = (mode: any) => {
        if (typeof mode === 'string') {
            const found = viewModes().find((m: any) => m.name === mode);
            if (found) {
                setViewMode(found);
                generateDates();
            }
     ...
     */
    const getDateInfo = (date: any, index: any, lastDate = null) => {
        const mode = viewMode();
        const lang = language();
