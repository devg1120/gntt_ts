...
export function createGanttConfigStore(options = {}) {
    ...
    const [subtaskHeightRatio, setSubtaskHeightRatio] = createSignal(options.subtaskHeightRatio || 0.5);
    ...
    const [expandedTasks, setExpandedTasks] = createSignal(new Set(options.expandedTasks || []));
    ...
    const updateOptions = (newOptions) => {
        ...
        if (newOptions.ganttEnd !== undefined)
            setGanttEnd(newOptions.ganttEnd);
        if (newOptions.unit !== undefined)
            setUnit(newOptions.unit);
        ...
        if (newOptions.ignoredFunction !== undefined)
            setIgnoredFunction(newOptions.ignoredFunction);
