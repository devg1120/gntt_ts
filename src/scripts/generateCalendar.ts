...
const fsCount = tasks.filter((t: any) => typeof t.dependencies === 'string').length;
// @ts-expect-error TS(2349): This expression is not callable.
const ssCount = tasks.filter((t: any) => t.dependencies?.type === 'SS').length;
// @ts-expect-error TS(2349): This expression is not callable.
const noDeps = tasks.filter((t: any) => !t.dependencies).length;
