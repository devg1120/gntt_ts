...
function getFormatters(lang: any) {
...
export default {
    ...
    parse(date: any, date_separator = '-', time_separator = /[.:]/) {
    ...
    to_string(date: any, with_time = false) {
        if (!(date instanceof Date)) {
    ...
    format(date: any, date_format = 'YYYY-MM-DD HH:mm:ss.SSS', lang = 'en') {
    ...
    diff(date_a: any, date_b: any, scale = 'day') {
    ...
    add(date: any, qty: any, scale: any) {
        qty = parseInt(qty, 10);
        const vals = [
    ...
    start_of(date: any, scale: any) {
        ...
        function should_reset(_scale: any) {
            // @ts-expect-error TS(7053): Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    ...
    convert_scales(period: any, to_scale: any) {
        const TO_DAYS = {
