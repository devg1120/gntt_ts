...
function createRandom(seed: any) {
    let state = seed;
    return function () {
...
function pick(random: any, array: any) {
    return array[Math.floor(random() * array.length)];
}

// Helper to get random int in range [min, max]
function randInt(random: any, min: any, max: any) {
...
// Lighten a hex color
function lightenColor(hex: any, percent = 20) {
...
export function generateSubtaskDemo(config = {}) {
    ...
    for (const [resource, resourceTasks] of tasksByResource) {
        ...
        resourceTasks.sort((a: any, b: any) => new Date(a.start) - new Date(b.start));
        ...
        for (let i = 1; i < resourceTasks.length; i++) {
            if (random() < cfg.dependencyChance) {
                ...
                const candidates = resourceTasks
                    .slice(0, i)
                    .filter((t: any) => new Date(t.end) <= new Date(currentTask.start));
