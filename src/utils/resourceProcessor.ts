 ...
 */
export function normalizeResources(rawResources: any) {
    if (!rawResources || rawResources.length === 0) {
        return [];
    }

    return rawResources.map((item: any) => {
 ...
 */
export function computeDisplayResources(resources: any, collapsedGroups: any) {
    const result = [];
 ...
 */
export function extractResourcesFromTasks(tasks: any) {
 ...
 */
export function buildGroupMap(resources: any) {
    const map = new Map();
    for (const item of resources) {
        if (item.type === 'group') {
 ...
 */
export function getResourcesInGroup(resources: any, groupId: any) {
    return resources.filter(
        (r: any) => r.type === 'resource' && r.group === groupId,
    );
}
 ...
 */
export function countResourcesPerGroup(resources: any) {
