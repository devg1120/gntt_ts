 ...
 */
export function Gantt(props: any) {
    // Create stores
    ...
    if (typeof window !== 'undefined') {
        ...
        // @ts-expect-error TS(2339): Property '__ganttConfig' does not exist on type 'W... Remove this comment to see the full error message
        window.__ganttConfig = ganttConfig;
        // @ts-expect-error TS(2339): Property '__ganttDateStore' does not exist on type... Remove this comment to see the full error message
    ...
    const modalRelationships = createMemo(() => {
        const id = modalTaskId();
        if (!id) return [];
        // @ts-expect-error TS(2339): Property 'from' does not exist on type 'never'.
        return relationships().filter((r) => r.from === id || r.to === id);
    });

    // Initialize tasks and compute positions
    const initializeTasks = (rawTasks: any, useResourceStore = true) => {
        ...
        // @ts-expect-error TS(2345): Argument of type 'Map<any, any> | null' is not ass... Remove this comment to see the full error message
    ...
    const handleTaskHover = (taskId: any, clientX: any, clientY: any) => {
    ...
    const handleContainerReady = (api: any) => {
        setContainerApi(api);
