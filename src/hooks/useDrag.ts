...
export function useDrag(options = {}) {
    // @ts-expect-error TS(2339): Property 'onDragStart' does not exist on type '{}'... Remove this comment to see the full error message
    ...
    const toSvgCoords = (clientX: any, clientY: any, svg: any) => {
        if (getSvgPoint) {
     ...
     */
    const startDrag = (e: any, state: any, data = {}) => {
        if (dragState() !== 'idle') {
            return;
        }
     ...
     */
    const createDragHandler = (state: any, dataOrGetter = {}) => {
        return (e: any) => {
            const data =
 ...
 */
export function movedEnough(startX: any, startY: any, currentX: any, currentY: any, threshold = 3) {
