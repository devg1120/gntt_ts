...
function iterateTasks(tasksObj: any) {
    if (!tasksObj) return [];
    if (tasksObj.entries) {
        // It's a Map
...
function getTask(tasksObj: any, id: any) {
 ...
 */
export function calculateRowLayouts(displayRows: any, config: any, expandedTasks: any, taskMap: any) {
    const {
        barHeight = 30,
        padding = 18,
    ...
    for (const row of displayRows) {
        ...
        if (expandedTasksInRow.length > 0) {
            ...
            const nonExpandedCount = resourceTasks.filter((t: any) => !t.parentId && t.type !== 'project' && !expandedTasks?.has(t.id)
            ).length;
        ...
        const overlapsRow = (task: any, row: any) => {
        ...
        const findRowForTask = (task: any, taskHeight: any) => {
            const taskStart = task._start ?? task.$bar?.x ?? 0;
            const taskEnd = task._end ?? (task.$bar?.x + task.$bar?.width) ?? taskStart + 1;
        ...
        for (const task of sortedTasks) {
            ...
            if (isExpanded) {
                ...
                if (task._children) {
                    ...
                    task._children.forEach((childId: any, index: any) => {
 ...
 */
export function calculateExpandedRowHeight(task: any, config: any, taskMap: any) {
    const {
        barHeight = 30,
        padding = 18,
        subtaskHeightRatio = 0.5,
 ...
 */
function computeSubtaskRows(childIds: any, taskMap: any) {
 ...
 * @returns {number} Total row height
 */
function calculateMixedLayoutHeight(task: any, config: any, taskMap: any) {
    const {
        barHeight = 30,
        padding = 18,
        subtaskHeightRatio = 0.5,
 ...
 */
export function findRowAtY(sortedRows: any, targetY: any) {
 ...
 */
export function rowLayoutsToSortedArray(rowLayouts: any) {
    const result = [];
 ...
 */
export function getVisibleRowRange(sortedRows: any, scrollY: any, viewportHeight: any, overscan = 2) {
 ...
 */
export function computeSubtaskY(subtaskIndex: any, parentContentY: any, layout: any, config: any, subtaskRow = 0) {
    const {
        barHeight = 30,
        padding = 18,
        subtaskHeightRatio = 0.5,
 ...
 * @returns {Map} rowLayouts - Map<rowId, { y, height, contentY, contentHeight }>
 */
export function calculateSimpleRowLayouts(displayRows: any, config: any) {
