 ...
 */
export function buildHierarchy(tasks: any) {
    ...
    const roots = tasks.filter((t: any) => !t.parentId);
    const queue = roots.map((t: any) => ({
        task: t,
        depth: 0
    }));
 ...
 */
export function collectDescendants(taskId: any, tasksObj: any) {
    const descendants = new Set();
    const queue = [taskId];
    // Support both Map and plain object
    const getTask = tasksObj.get ? (id: any) => tasksObj.get(id) : (id: any) => tasksObj[id];
 ...
 */
export function isHiddenByCollapsedAncestor(taskId: any, tasksObj: any, collapsedSet: any) {
    const getTask = tasksObj.get ? (id: any) => tasksObj.get(id) : (id: any) => tasksObj[id];
 ...
 */
export function getAncestors(taskId: any, tasksObj: any) {
    const getTask = tasksObj.get ? (id: any) => tasksObj.get(id) : (id: any) => tasksObj[id];
    const ancestors = [];
    let task = getTask(taskId);
