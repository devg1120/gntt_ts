...
export function createGanttConfigStore(options = {}) {
    // Time configuration
    // @ts-expect-error TS(2339): Property 'ganttStart' does not exist on type '{}'.
    const [ganttStart, setGanttStart] = createSignal(options.ganttStart || new Date());
    // @ts-expect-error TS(2339): Property 'ganttEnd' does not exist on type '{}'.
    ...
    // @ts-expect-error TS(2339): Property 'columnWidth' does not exist on type '{}'... Remove this comment to see the full error message
    const [columnWidth, setColumnWidth] = createSignal(options.columnWidth || options.column_width || 45);
    // @ts-expect-error TS(2339): Property 'barHeight' does not exist on type '{}'.
    const [barHeight, setBarHeight] = createSignal(options.barHeight || options.bar_height || 30);
    // @ts-expect-error TS(2339): Property 'headerHeight' does not exist on type '{}... Remove this comment to see the full error message
    const [headerHeight, setHeaderHeight] = createSignal(options.headerHeight || 75);
    // @ts-expect-error TS(2339): Property 'padding' does not exist on type '{}'.
    const [padding, setPadding] = createSignal(options.padding || 18);
    // @ts-expect-error TS(2339): Property 'barCornerRadius' does not exist on type ... Remove this comment to see the full error message
    ...
    // @ts-expect-error TS(2339): Property 'readonlyDates' does not exist on type '{... Remove this comment to see the full error message
    const [readonlyDates, setReadonlyDates] = createSignal(options.readonlyDates || false);
    // @ts-expect-error TS(2339): Property 'readonlyProgress' does not exist on type... Remove this comment to see the full error message
    const [readonlyProgress, setReadonlyProgress] = createSignal(options.readonlyProgress || false);
    // @ts-expect-error TS(2339): Property 'showExpectedProgress' does not exist on ... Remove this comment to see the full error message
    const [showExpectedProgress, setShowExpectedProgress] = createSignal(options.showExpectedProgress || false);
    // @ts-expect-error TS(2339): Property 'autoMoveLabel' does not exist on type '{... Remove this comment to see the full error message
    const [autoMoveLabel, setAutoMoveLabel] = createSignal(options.autoMoveLabel || false);

    // Ignored dates (weekends, holidays)
    // @ts-expect-error TS(2339): Property 'ignoredDates' does not exist on type '{}... Remove this comment to see the full error message
    const [ignoredDates, setIgnoredDates] = createSignal(options.ignoredDates || []);
    // @ts-expect-error TS(2339): Property 'ignoredFunction' does not exist on type ... Remove this comment to see the full error message
    ...
    const [subtaskHeightRatio, setSubtaskHeightRatio] = createSignal(
        // @ts-expect-error TS(2339): Property 'subtaskHeightRatio' does not exist on ty... Remove this comment to see the full error message
    ...
    // @ts-expect-error TS(2339): Property 'renderMode' does not exist on type '{}'.
    ...
    const [expandedTasks, setExpandedTasks] = createSignal(
        // @ts-expect-error TS(2339): Property 'expandedTasks' does not exist on type '{... Remove this comment to see the full error message
        new Set(options.expandedTasks || [])
    );

    // Expansion management methods
    const isTaskExpanded = (taskId: any) => expandedTasks().has(taskId);
    ...
    const expandAllTasks = (taskIds: any) => {
