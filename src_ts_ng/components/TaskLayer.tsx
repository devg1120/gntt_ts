 ...
 */
export function TaskLayer(props: any) {
     ...
     * Handle task hover.
     */
    const handleHover = (taskId: any, clientX: any, clientY: any) => {
     ...
     */
    const handleCollectDependents = (taskId: any) => {
     ...
     */
    const handleClampBatchDelta = (batchOriginals: any, proposedDeltaX: any) => {
        const getTask = props.taskStore?.getTask?.bind(props.taskStore);
        // Get columnWidth for lag conversion (lag is in days, needs pixels)
        const columnWidth = props.ganttConfig?.columnWidth?.() ?? 45;
     ...
     */
    const handleCollectDescendants = (taskId: any) => {
     ...
     */
    const handleToggleCollapse = (taskId: any) => {
    ...
    const isTaskExpanded = (taskId: any) => {
    ...
    const getTaskPosition = (taskId: any) => {
        ...
        <g class="task-layer" style="contain: layout style;">
            ...
            <g class="summary-layer" style="contain: layout style;">
                ...
                <Index each={pooledSummaryIds()}>
                    // @ts-expect-error TS(7026): JSX element implicitly has type 'any' because no i... Remove this comment to see the full error message
                    {(taskId: any) => <g style={{ display: taskId() ? 'block' : 'none' }}>
                        <SummaryBar
                            taskId={taskId}
            ...
            <g class="expanded-layer" style="contain: layout style;">
                ...
                <For each={splitTaskIds().expandedIds}>
                    {(taskId: any) => <ExpandedTaskContainer
                        taskId={taskId}
                        taskStore={props.taskStore}
                        ganttConfig={props.ganttConfig}
